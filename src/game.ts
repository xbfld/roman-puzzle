...
// 다음 레벨 퀘스트 문자열 가져오기
export function getNextQuest(level: number): string {
  return toRoman(level);
}

// 현재 밟아야 할 문자 가져오기
export function getCurrentRequiredChar(state: GameState): RomanChar | null {
  if (!state.isOnQuest) {
    // 퀘스트 시작 전: 첫 글자 필요
    return state.currentQuest[0] as RomanChar;
  }
  if (state.questProgress >= state.currentQuest.length) {
    return null; // 모든 문자를 밟음 - 쉼터로 돌아가야 함
  }
  return state.currentQuest[state.questProgress] as RomanChar;
}
...
// 방향으로 새 위치 계산
export function getNewPosition(pos: Position, direction: Direction): Position {
  switch (direction) {
    case 'up':
...
// 특정 위치로 이동 가능한지 확인
export function canMoveTo(state: GameState, targetPos: Position): boolean {
  // 인접한 위치만 이동 가능
...
export function move(state: GameState, direction: Direction): MoveResult {
  ...
  let leveledUp = false;
  let autoPlacedTile: RomanChar | null = null;
...
// 게임 리셋
export function resetGame(viewportSize: number = 11): GameState {
...
export interface GameStatus {
  ...
  isComplete: boolean; // 퀘스트 문자를 모두 밟았는지
  isGameOver: boolean;
}
...
// 타임라인 초기화
export function createTimeline(viewportSize: number = 11): GameTimeline {
  return {
    viewportSize,
    moves: [],
    currentIndex: 0,
...
export function getStateAtIndex(timeline: GameTimeline, targetIndex: number): GameState {
...
// 타임라인에 이동 추가
export function addMoveToTimeline(
  timeline: GameTimeline,
  direction: Direction,
  currentState: GameState
): { timeline: GameTimeline; moveResult: MoveResult } {
...
// 강력 undo (이전 레벨 시작점으로)
export function strongUndoTimeline(timeline: GameTimeline): GameTimeline {
...
// 강력 redo (다음 레벨 시작점으로)
export function strongRedoTimeline(timeline: GameTimeline): GameTimeline {
...
// 타임라인을 특정 인덱스로 이동
export function seekTimeline(timeline: GameTimeline, targetIndex: number): GameTimeline {
